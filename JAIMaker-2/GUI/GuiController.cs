        ...
        private byte[] LoadEmbeddedShaderCode(ResourceFactory factory, string name, ShaderStages stage)
        {
            switch (factory.BackendType)
            {
                case GraphicsBackend.Direct3D11:
                    {
                        string resourceName = "JAIMaker_2.Shaders.HLSL." + name + ".hlsl.bytes";
                        return GetEmbeddedResourceBytes(resourceName);
                    }
                case GraphicsBackend.OpenGL:
                    {
                        string resourceName = "JAIMaker_2.Shaders.GLSL." + name + ".glsl";
                        return GetEmbeddedResourceBytes(resourceName);
                    }
                case GraphicsBackend.Vulkan:
                    {
                        string resourceName = "JAIMaker_2.Shaders.SPIR_V." + name + ".spv";
                        return GetEmbeddedResourceBytes(resourceName);
                    }
                case GraphicsBackend.Metal:
                    {
                        string resourceName = "JAIMaker_2.Shaders.Metal." + name + ".metallib";
                        return GetEmbeddedResourceBytes(resourceName);
                    }
                default:
