...
namespace JAIMaker_2.JAIM
{
    ...
    public class JInstrumentEnvelopev1
    {
        ...
        private void loadFromStream(BeBinaryReader reader)
        {
            ...
            JEnvelopeVector last = null;
            for (int i = 0; i < count; i++)
            {
                var newE = (points[i] = new JEnvelopeVector { Mode = reader.ReadUInt16(), Delay = reader.ReadUInt16(), Value = reader.ReadInt16() });
                if (last != null)
                    last._next = newE;
                last = newE;
            }
